...
def _wait_jobs_images(*, browser_name=None) -> bool:
    ...
    while time.time() - t0 < JOBS_IMAGES_MAX_SEC:
        if find_creator_image(IMAGE_JOBS_IMAGES, browser_name=browser_name):
            print(f"[post-exito] Imagen «{IMAGE_JOBS_IMAGES}» encontrada.")
            return True
...
def _wait_login_with_email_image(*, browser_name=None) -> bool:
    ...
    while time.time() - t0 < LOGIN_WITH_EMAIL_MAX_SEC:
        if find_creator_image(IMAGE_LOGIN_WITH_EMAIL, browser_name=browser_name):
            print(f"[post-exito] Imagen «{IMAGE_LOGIN_WITH_EMAIL}» encontrada.")
            return True
...
def _wait_try_premium(*, browser_name=None) -> bool:
    ...
    while time.time() - t0 < TRY_PREMIUM_MAX_SEC:
        if find_creator_image(IMAGE_TRY_PREMIUM, browser_name=browser_name):
            print(f"[post-exito] Imagen «{IMAGE_TRY_PREMIUM}» encontrada.")
            return True
